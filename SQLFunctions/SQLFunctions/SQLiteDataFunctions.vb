...
Public Class SQLiteDataFunctions
    ...
    Public Sub BulkInsert(ByVal DBName As String, ByVal DT As DataTable, ByVal TableName As String)
        ...
        'Dim transaction = Conn.BeginTransaction 'This begins the bulk insert

        Using MyConn = New SQLite.SQLiteConnection(GetConnectionString(DBName))
            Using SQLCmd = New SQLiteCommand(MyConn)
                MyConn.Open() 'MUST declare this open at this stage or it returns a Invalid Operation Error due to connection being closed
                Using transaction = MyConn.BeginTransaction()
                    Try
                        For row As Integer = 1 To DT.Rows.Count - 1 'cycle through each row
                            For Col As Integer = 0 To DT.Columns.Count - 1
                                'Console.WriteLine(DT.Rows(row).Item(DT.Columns.Item(Col)) & "  " & DT.Columns(Col).ColumnName)
